...
export class AuditTrailService {
  ...
  async findAll(query: AuditQueryDto = {}) {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to fetch audit logs due to an internal server error.',
      );
    }
  }
  ...
  async findByEntity(entityType: string, entityId: string) {
    try {
      ...
      this.logger.error(
        `❌ Failed to fetch audit logs for entity ${entityType}:${entityId}:`,
        error.stack || error,
      );
  ...
  private parseDate(dateString: string, paramName: string): Date {
    ...
    if (isNaN(date.getTime())) {
      throw new BadRequestException(
        `Invalid date format for '${paramName}'. Please use a valid ISO 8601 date string.`,
      );
    }
    return date;
  }
