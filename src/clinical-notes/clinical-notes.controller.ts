...
import {
  ...
  CreateNoteSuggestionDto,
  QueryClinicalNotesDto,
  PatientAutoPopulateResponseDto,
...
export class ClinicalNotesController {
  constructor(private readonly service: ClinicalNotesService) {} // 1. Add clinical note (Doctor/Admin/SuperAdmin)

  @Post()
  @Roles(Role.DOCTOR, Role.ADMIN, Role.SUPERADMIN)
  @ApiOperation({
    summary: 'Add a clinical note for a patient (supports extended form via extendedData)',
  })
  ...
  async getNotes(@Param('patientId') patientId: string, @Query() query: QueryClinicalNotesDto) {
  ...
  async findAll(@Query() query: QueryClinicalNotesDto, @GetUser() user: any) {
  ...
  @ApiResponse({
    ...
    type: PatientAutoPopulateResponseDto,
  })
