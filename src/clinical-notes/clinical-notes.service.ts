...
export class ClinicalNotesService {
  ...
  async addNote(patientId: string, userId: string, role: Role, dto: CreateClinicalNoteDto) {
    ...
    try {
      ...
      if (!patientExists) throw new NotFoundException('Patient not found'); // 3. split legacy top-level fields and extended fields
      ...
      const note = await this.prisma.clinicalNote.create({
        data: {
          ...
          status: 'APPROVED', // FIX 2: Use Prisma.JsonNull for null value assignment
  ...
  async editNote(
    ...
    try {
      ...
      if (note.patientId !== patientId)
        throw new ForbiddenException('This note does not belong to the specified patient.'); // const oldData = { ...note }; // Original: Capture pre-update state for audit (kept for context)
      ...
      const { observations, doctorNotes, treatmentPlan, extra, ...extendedFields } = dto; // FIX 3: Single declaration of updates with correct Prisma type
      const updates: Prisma.ClinicalNoteUpdateInput = {}; // Assign standard fields if provided
      if (observations !== undefined) updates.observations = observations;
      if (doctorNotes !== undefined) updates.doctorNotes = doctorNotes;
      if (treatmentPlan !== undefined) updates.treatmentPlan = treatmentPlan; // Merge extended data
      ...
      const mergedExtended = { ...existingExtended, ...providedExtended }; // Only update extendedData if there are fields provided in the DTO
      if (Object.keys(providedExtended).length > 0) {
        // FIX 4: Use Prisma.JsonNull for empty object to satisfy Prisma's JSON type
        updates.extendedData = Object.keys(mergedExtended).length
          ? mergedExtended
          : Prisma.JsonNull;
      }
      // The rest of the duplicated logic blocks below this point were entirely removed.

      updates.updatedAt = new Date(); // 4. Perform update
  ...
  async getNotes(patientId: string, query: QueryClinicalNotesDto) {
    try {
      ...
      const skip = (page - 1) * limit; // Optional: Check if patient exists first for clearer error message,
  ...
  async findAll(query: QueryClinicalNotesDto, user: { role: Role }) {
    ...
    try {
      const allowed = new Set<Role>([Role.DOCTOR, Role.NURSE, Role.ADMIN, Role.SUPERADMIN]);
      if (!allowed.has(user.role))
        throw new ForbiddenException('Insufficient permissions to list all clinical notes');
    ...
    try {
      ...
      if (!allowed.has(role))
        throw new ForbiddenException(
          'Only nurses, admins, or superadmins can add note suggestions',
        );
      ...
      this.prisma.auditLog
        ...
        .catch((e) => this.logger.warn(`Audit log failed for ${op}: ${e.message}`)); // send mail notification (best-effort, catch error to prevent main failure)
  ...
  async approveSuggestion(patientId: string, suggestionId: string, userId: string, role: Role) {
    ...
    try {
      ...
      const allowed = new Set<Role>([Role.DOCTOR, Role.ADMIN, Role.SUPERADMIN]);
      if (!allowed.has(role))
        throw new ForbiddenException('Only doctors or admins can approve suggestions'); // 2. Validation
      ...
      if (suggestion.patientId !== patientId)
        throw new ForbiddenException('Suggestion does not belong to this patient'); // 3. Transaction: Update suggestion status and create new clinical note
      ...
      this.prisma.auditLog
        ...
        .catch((e) => this.logger.warn(`Audit log failed for ${op}: ${e.message}`)); // best-effort notify nurse

      this.mailer
        .sendMail(
          'nurse@hospital.com',
          'Clinical Note Suggestion Approved',
          `Your suggestion for ${suggestion.patientId} was approved.`,
        )
